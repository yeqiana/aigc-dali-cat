...
def _env_summary() -> dict:
    ...
    summary["failure_injection"] = {"mysql_host": REFUSED_MYSQL_HOST, "mysql_port": REFUSED_MYSQL_PORT}
    return summary
...
def build_drill(args) -> CanaryDrill:
    redis_client = RedisConnection(**storage_config.redis_connection_kwargs()).client
    redis_client.ping()
    mysql = MySqlConnection(**storage_config.mysql_connection_kwargs())
    mysql.health_check()
    broken = CanaryProbe(
