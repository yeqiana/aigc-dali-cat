    ...
    <main className="storyos-shell min-h-screen bg-[var(--bg-app)] text-[var(--text-primary)] p-4 lg:p-6">
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <section className="storyos-surface overflow-hidden">
          ...
          {executions.length > 0 ? (
            <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-xs"><thead className="h-9 border-b border-[var(--border-subtle)] text-[var(--text-tertiary)]"><tr><th className="px-3 text-left">Execution ID</th><th className="px-3 text-left">Status</th><th className="px-3 text-left">Trace ID</th></tr></thead><tbody>{executions.map((row) => <tr key={row.id} className="h-11 border-b border-[var(--border-subtle)] last:border-b-0 hover:bg-[var(--bg-hover)]"><td className="px-3 font-mono text-[var(--text-primary)]">{row.id}</td><td className="px-3 font-mono">{row.status}</td><td className="px-3 font-mono text-[var(--text-tertiary)]">{row.trace_id || '-'}</td></tr>)}</tbody></table></div>
