...
V1.1 禁止再创建第二套 `episode.yaml stage`。
...
└── story-gates.json       # 故事/视觉/字幕/锁图门禁证据，不保存阶段
```
...
`story-gates.json` 永远不能覆盖 `current_state`，也不能自建另一套 stage。
...
V1.1 会同时生成三个 meta 文件。

旧剧集不批量伪造 `story-gates.json`；旧剧集重新进入制作时执行：
...
本地发布前不要使用 `--metadata-only`。

## 8. 边界

机器门禁可以判断“证据是否齐、图片尺寸/数量/hash 是否正确”，但不能替代人工/多模态判断：
