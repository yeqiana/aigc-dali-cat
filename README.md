...
新篇流程：候选题 → 最近5篇账号级同质化检查 → 人群匹配 → 四张视觉准入 → 分镜 → 出图 → 字幕 → 制作完成度验收 → 发布前九项传播结构评分 → 搜索资产与治理检查 → publish 定稿 → 时间窗口选择 → 1h 冷启动快照 → 6h/24h/48h/7d L1–L4 正式漏斗复盘 → 更新账号相似度与题材基线，并在剧集索引登记状态。发布时间实验继续沿用 V1.3 建立的 A/B/C/D 四窗口，由 V1.4 接管执行口径。
