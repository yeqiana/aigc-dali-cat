...
- 传播评分与发布后漏斗执行细则：[standards/抖音推流评分与发布后漏斗规范_V1.3.md](standards/抖音推流评分与发布后漏斗规范_V1.3.md)（仅为主规范执行细则）
...
新篇流程：按规范产出分镜 → 出图 → 字幕 → 制作完成度验收 → 发布前九项传播结构评分 → publish 定稿 → 做发布时间实验时采 1h 冷启动快照 → 6h/24h/48h/7d L1–L4 正式漏斗复盘，并在剧集索引登记状态。发布时间实验按 V1.3 四窗口轮换，不再固定连续 3 篇 19:00–19:15。
