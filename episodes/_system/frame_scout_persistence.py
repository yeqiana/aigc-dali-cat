...
def _record(ep: Path, payload: dict) -> dict:
    ...
    if frame < 1:
        raise ValueError("frame scout requires frame")
    decision = str(payload.get("decision") or "").strip()
    if not decision:
        raise ValueError("frame scout requires decision")
    return {
...
def save(ep: Path, payload: dict) -> dict:
    ...
    if frame in (None, ""):
        raise ValueError("frame scout save requires frame")
    path = _legacy_path(ep, frame)
    if mode != "mysql":
