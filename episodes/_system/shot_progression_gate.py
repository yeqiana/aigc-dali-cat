#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Capture-setup diversity + anomaly-logic escalation + human-action ladder."""
...
def prepare(ep,force=False):
    ...
    for n in range(1,total+1):
        rows.append({"frame":f"{n:02d}","camera_position":"","subject_distance":"","primary_subject":"",
          "action":"","visual_function":"","capture_purpose":"","pov_mode":"","location_zone":"",
          "anomaly_logic_stage":"ordinary","human_action_stage":"ordinary","new_information":False,
          "continuity_exception_reason":""})
    d={"schema_version":1,"status":"DRAFT","anomaly_applicable":True,"anomaly_exception_reason":"",
       "rules":{"max_identical_setup_consecutive":2,"min_unique_setups_in_5_frames":3,
...
def validate(ep,require_locked=True):
    ...
    if d.get("schema_version")!=1:e.append("shot progression schema_version must be 1")
    ...
    for row in rows:
        ...
        sig=_sig(row)
        same_run=same_run+1 if sig==prev_sig else 1
    ...
    if not applicable:
        ...
        if conf>=0:
            ...
            for idx in range(conf+1,len(normalized)):
                ...
                if passive_run>2:
                    e.append(f"HUMAN_ACTION_STALL:{normalized[idx][0]:02d}")
                    break
    if total>=10:
...
def self_test():
    ...
    assert "act" in HUMAN_STAGES
    print("SHOT PROGRESSION GATE SELF-TEST PASS")
