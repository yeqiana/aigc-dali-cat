...
RULES: tuple[tuple[str, StorageDecision], ...] = (
    ...
    ("_archive/relocations.json", StorageDecision(FILE, "KEEP", "archive_metadata", "archive relocation ledger is repository history, not runtime authority")),
    ("_system/MODULE_INDEX.json", StorageDecision(FILE, "KEEP", "system_manifest", "system module index is a repository manifest, not runtime authority")),
    ("meta/tests/**", StorageDecision(FILE, "MOVE_TO_TEST_FIXTURES", "test_fixture", "test fixtures must not be classified as production review/contract evidence")),
    ...
    ("meta/runtime/reviews/*-request.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "runtime_review", "canonical durable Runtime Review request/current-attempt evidence")),
    ("meta/runtime/reviews/*-request.json.pre-*.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "review_revision", "historical pre-change Runtime Review request snapshot")),
    ("meta/runtime/reviews/*-request.*.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "review_revision", "versioned or superseded Runtime Review request snapshot")),
    ("meta/runtime/reviews/*-request-superseded-*.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "review_revision", "superseded Runtime Review request snapshot")),
    ...
    ("meta/series-character-identity.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "series_identity_contract", "series character identity is durable production input metadata")),
    ("meta/v21-narrative-upgrade.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "migration", "narrative upgrade record is durable migration evidence")),
    ("meta/asset-manifest.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "artifact", "asset manifest belongs with durable artifact metadata")),
    ("meta/chapter-lock.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "episode_contract", "chapter lock is durable creative authority evidence")),
    ("meta/episode-blueprint.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "episode_contract", "episode blueprint is a durable production input contract")),
    ("meta/episode-abandoned.json", StorageDecision(FILE, "KEEP", "archive_metadata", "abandoned Episode marker remains historical archive metadata and is never active runtime authority")),
    ...
    ("*/contracts/*.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "episode_contract", "legacy Episode contract file discovered from repository-root inventory")),
    ...
    ("meta/preproduction-validation.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "validation", "preproduction validation run and check results")),
    ("meta/validation/*_validation_report.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "validation", "machine validation run and check results; raw report remains an export/evidence view")),
    ("meta/post-publish-metrics.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "metrics", "post-publish checkpoint facts; queryable metric values belong in metric snapshots")),
    ("meta/post-publish-review.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "review", "derived post-publish review summary; recomputable from metric snapshots")),
    ("meta/next-story-learning.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "learning", "evidence-only learning projection; not creative authority")),
    ("meta/publish-event.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "publication", "actual publication fact outside the frozen release manifest")),
    ("meta/release-package.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "release", "release package metadata")),
    ("meta/DELEGATED_AUTO_REPORT.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "release", "delegated automation delivery report")),
    ...
    ("*/publish/**/assembly-audit.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "release", "publish assembly audit discovered from repository-root inventory")),
    ...
    ("meta/runtime/metrics/*.json", StorageDecision(MYSQL, "REMOVE_AFTER_CUTOVER", "metrics", "runtime metric projection; full payload may be an Artifact document")),
    ("meta/runtime/approvals/*.json", StorageDecision(FILE, "KEEP", "approval_document", "full approval evidence document referenced by MySQL")),
    ("meta/runtime/releases/*.json", StorageDecision(FILE, "KEEP", "release_document", "full release evidence document referenced by MySQL")),
    ("meta/runtime/review-documents/*.json", StorageDecision(FILE, "KEEP", "review_document", "full review evidence document referenced by MySQL")),
    ("meta/runtime/checkpoint-events/*.json", StorageDecision(FILE, "KEEP", "checkpoint_document", "large checkpoint event evidence referenced by MySQL")),
