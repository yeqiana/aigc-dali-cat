...
def test_plan_migrates_only_operational_and_derived(monkeypatch, tmp_path):
    ...
    assert by_path["meta/runtime/contracts/frames/01.json"]["action"] == "DEFER_CONSUMER_MIGRATION"
...
def test_execute_copy_defers_mysql_owned_frame_cache_and_queue(monkeypatch, tmp_path):
    ep = _configure(monkeypatch, tmp_path)
    ...
    assert actions["meta/runtime/contracts/frames/01.json"] == "DEFER_CONSUMER_MIGRATION"
    report = runtime_workspace_migration.execute_copy(ep, plan)
    assert report["status"] == "PASS"
    assert report["copied"] == 0 and report["verified"] == 0
    assert not runtime_workspace.workspace_path(ep, "meta/runtime/contracts/frames/01.json").exists()
